...
class Hyperparameters:
    ...
    moe_aux_weight: float = float(os.environ.get("MOE_AUX_WEIGHT", 0.01))
...
class MoEMLP(nn.Module):
    """
    Mixture-of-Experts MLP with top-k routing (MLX version).

    Full-dispatch approach: every token is run through every expert, then we
    select/weight the top-k outputs. This keeps array shapes static for
    mx.compile compatibility.
    """
    ...
    def __init__(self, dim: int, mlp_mult: int, num_experts: int, top_k: int):
        ...
        self.top_k = top_k
        # Router: CastedLinear to keep weights in fp32 for optimizer quality.
        self.router = CastedLinear(dim, num_experts)
        # Each expert is a small relu^2 MLP.
        self.experts = [MLP(dim, mlp_mult) for _ in range(num_experts)]
        # aux_loss is set during forward and read by the GPT loss method.
        self._aux_loss = mx.array(0.0, dtype=mx.float32)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # Router logits in float32 for stability.
        router_logits = self.router(x_flat.astype(mx.float32))  # (N, num_experts)
        router_probs = mx.softmax(router_logits, axis=-1)  # (N, num_experts)

        # Load-balancing auxiliary loss (Switch Transformer style).
        # f_i: fraction of tokens where expert i is top-1 (no gradient through argmax).
        top1_indices = mx.argmax(router_logits, axis=-1)  # (N,)
        # One-hot encode via eye indexing, then mean to get fraction per expert.
        f = mx.mean(mx.eye(self.num_experts, dtype=mx.float32)[top1_indices], axis=0)  # (num_experts,)
        # P_i: mean probability per expert (has gradients).
        P = mx.mean(router_probs, axis=0)
        self._aux_loss = self.num_experts * mx.sum(mx.stop_gradient(f) * P)

        # Top-k gating.
        topk_indices = mx.argpartition(-router_probs, kth=self.top_k, axis=-1)[:, :self.top_k]  # (N, top_k)
        # Gather the top-k probabilities.
        topk_weights = mx.take_along_axis(router_probs, topk_indices, axis=-1)  # (N, top_k)
        topk_weights = topk_weights / mx.sum(topk_weights, axis=-1, keepdims=True)

        # Full dispatch: run all experts on all tokens.
        expert_outputs = mx.stack(
            [expert(x_flat) for expert in self.experts], axis=1
        )  # (N, num_experts, dim)

        # Gather top-k expert outputs.
        gather_idx = mx.expand_dims(topk_indices, axis=-1)  # (N, top_k, 1)
        gather_idx = mx.broadcast_to(gather_idx, (N, self.top_k, dim))  # (N, top_k, dim)
        selected = mx.take_along_axis(expert_outputs, gather_idx, axis=1)  # (N, top_k, dim)

        # Weighted combination.
        out = mx.sum(selected * mx.expand_dims(topk_weights, axis=-1).astype(selected.dtype), axis=1)  # (N, dim)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, num_experts: int = 0, moe_top_k: int = 1, moe_aux_weight: float = 0.01):
        super().__init__()
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        # Accumulate MoE auxiliary load-balancing loss.
        aux_loss = mx.array(0.0, dtype=mx.float32)
        num_moe_layers = 0
        for block in self.blocks:
            if isinstance(block.mlp, MoEMLP):
                aux_loss = aux_loss + block.mlp._aux_loss
                num_moe_layers += 1
        if num_moe_layers > 0:
...
def main() -> None:
    ...
    if args.num_experts > 1:
        log(f"moe:enabled num_experts:{args.num_experts} top_k:{args.moe_top_k} aux_weight:{args.moe_aux_weight}")
    log(
